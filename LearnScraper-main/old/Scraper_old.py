...
def navToCourse(cCode):
    ...
    search.clear() ###
    search.send_keys(cCode[:-1]) # stops the scraper selecting its own search.

    ##selects the first (and hopefully only) course available after search
    course = WebDriverWait(driver, 10, 0.1).until(EC.presence_of_element_located((By.XPATH, "//*[contains(text(), '"+cCode+"')]")))
...
def getGrades():
    ...
    download = buttons.find_elements(By.XPATH, "//*[contains(text(), 'Download')]")
    download[2].click()

#🍂 used when collecting diaries to identify how stars and wishes are seperated
def numberOfStartingSymbols(paras):
    counter = 0


    for p in paras:
        if len(p.text.lstrip()) <= 4:
            continue
        char1st = p.text.lstrip()[0]

        if(is_emoji(char1st) | (char1st in "*?-")):
            counter += 1
...
#🍂 used when collecting discussions
#   attempts to categorises stars and wishes, still collects raw.
#   stars and wishes are marked index issue if it cannot be categorized.
def readDiaryEntry(ent, currentDiary, courseCode):
    print(currentDiary)
    print(ent.text)
    print("-------------------")
    ...
    print(name)
    progText.set(f"reading entry for {name}")
    textHolder = ent.find_element(By.ID, "bb-editor-textbox")
    paras = textHolder.find_elements(By.XPATH, ".//*")
    paras = (textHolder.find_elements(By.TAG_NAME, "p") + textHolder.find_elements(By.TAG_NAME, "h4") + 
             textHolder.find_elements(By.TAG_NAME, "pre") + textHolder.find_elements(By.TAG_NAME, "li"))

    date = ent.find_element(By.XPATH, ".//*[@value='date']").text
    #print(date)
    pass
    ...
    if numberOfStartingSymbols(paras) < 4:
        textOnly = True
    ...
    for p in paras:
        ...
        if len(p.text.lstrip()) <= 4: # could now be redundant (switch backt to empty or not)
            continue
        
        char1st = p.text.lstrip()[0]
        #print(raw)
        #print(f"first char is {char1st}")
        #print(f"textOnly is {textOnly}, is emoji is {is_emoji(char1st)} and symbol check is {(char1st in '*?-')}")
        #print((is_emoji(char1st) | (char1st in "*?-")) | textOnly)
        if is_emoji(char1st) | (char1st in "*?-") | textOnly:
            #print("array")
            #print(starWishArr)
            #print("para text")
            #print(p.text)
            #print(f"true false {p.text in starWishArr}")
            if p.text in starWishArr: # may be redundant with new methods
                print("passed")
                continue
            
            if index < 4:
                starWishArr[index] = p.text

            index += 1

    if index != 4:
        #print(f"{index} issue")
        starsAndWish.loc[len(starsAndWish)] = [name,courseCode,currentDiary,f"indexIssue{ index}","indexIssue","indexIssue","indexIssue",raw,date]
        #print(raw)
        ...
        for p in paras:
            ...
            #print(f"'{p.text}' {(is_emoji(char1st) | (char1st in '*?'))}")
        ...
        #print(f"[star] {starWishArr[0]}")
        #print(f"[star] {starWishArr[1]}")
        #print(f"[star] {starWishArr[2]}")
        #print(f"[wish] {starWishArr[3]}")
        pass
    else:
        starsAndWish.loc[len(starsAndWish)] = [name,courseCode,currentDiary,starWishArr[0],starWishArr[1],starWishArr[2],starWishArr[3],raw,date]
        #print(f"[star] {starWishArr[0]}")
        #print(f"[star] {starWishArr[1]}")
        #print(f"[star] {starWishArr[2]}")
        #print(f"[wish] {starWishArr[3]}")
...
def downloadPage(courseCode):
    ...
    for sType in sessionTypes:
        sName = sType.text.replace(' ','_').splitlines()[0]
...
def getdiscussion(courseCode):
    ...
    scroll_origin = ScrollOrigin.from_element(driver.find_element(By.CLASS_NAME,"course"))
    containers = scrollUntilAllClass(scroll_origin, By.CLASS_NAME,"click-to-invoke-container")

    for cont in containers: #opens all containers
        buttons = cont.find_elements(By.TAG_NAME, "button")
        #folders are identified by the fact they have 3 buttons
        if(len(buttons) == 3):
            

            panelName = buttons[1].text
            print(panelName, "name")
            if panelName in diaryBlacklist.get(courseCode, []):
                print(panelName,"banned for ", courseCode)
                continue

            
            waitClick(cont,100)
    
    allContainers = driver.find_elements(By.CLASS_NAME,"click-to-invoke-container")
    discussions = []
    for cont in allContainers:
        buttonCount = cont.find_elements(By.TAG_NAME, "button")
        if(len(buttonCount) == 2):
            discussions.append(cont)
    

    for dis in discussions:
        WebDriverWait(driver, 10, 0.1).until(EC.element_to_be_clickable(dis))
        disName = dis.find_element(By.TAG_NAME,"a")
        print(disName.text, "disname")
        progText.set(f"opening {disName.text}")

        waitClick(disName,100)
        
        scrollPoint = WebDriverWait(driver, 10, 0.1).until(EC.presence_of_element_located((By.XPATH, '/html/body/div[1]/div[2]/bb-base-layout/div/main/div[5]/div/div/div/div/div/div/section/div[2]/div[2]/div[1]/div/div/div/div')))
        scroll_origin = ScrollOrigin.from_element(scrollPoint)


        entries = scrollUntilAllClass(scroll_origin,By.CLASS_NAME, "comment-entry")


        for ent in entries:
            readDiaryEntry(ent, disName.text, courseCode)

        

        driver.find_element(By.XPATH,"/html/body/div[1]/div[2]/bb-base-layout/div/main/div[5]/div/button").click() #exists
    starsAndWish.to_csv("starsAndWishes.csv", index=False)
...
def startVals():
    ...
    # password = None #To stop the password appearing on the debug menu


#🍂 the following courses where excluded from list_of_courses.txt
#    for various reasons however they do use the 3 stars 1 wish method.

#'PUHR111162023-4SS1FLEX, PUHR111032023-4SV1FLEX 🍂 did not have access for PUHR courses
#HEIN110482023-4 and HEIN110392023-4  🍂 These courses had their discussion pages restricted

#HEIN110392022-3        🍂These courses were not included as
#HEIN110452022-3           as they were on the old learn
#HEIN110502022-3
#HEIN110592022-3
#HEIN110452022-3
#HEIN110502022-3
...
mode = tk.IntVar(value=Modes.Attendance.value)
...
textInputLabel = tk.Label(text="names or ids of courses, one per row")
...
textInputLabel.grid(row=1, column=1,columnspan=2,rowspan=6,padx=5, pady=5, sticky="nswe")
textInput.grid(row=1, column=1,columnspan=2,rowspan=6,padx=5, pady=5, sticky="nswe")
